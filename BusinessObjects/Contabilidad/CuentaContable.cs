...
public class CuentaContable(Session session) : EntidadBase(session)
{
    ...
    private CuentaContable? _cuentaPadre;
    ...
    [RuleRequiredField("RuleRequiredField_Cuenta_Codigo", DefaultContexts.Save, CustomMessageTemplate = "El Código de la CuentaContable es obligatorio")]
    [RuleUniqueValue]
        ...
        set
        {
            if (value != null && value.Contains('.'))
            {
                ...
                if (partes.Length == 2)
                {
                    ...
                    int totalPadding = companyInfo?.PaddingCuentaContable ?? 10;
    ...
    private void ActualizarPadre()
    {
        ...
        if (codigoPadre != null)
        {
            CuentaPadre = Session.FindObject<CuentaContable>(PersistentCriteriaEvaluationBehavior.InTransaction, new BinaryOperator(nameof(Codigo), codigoPadre));
        }
        else
        {
            CuentaPadre = null;
    ...
    public XPCollection<CuentaContable> CuentasHijas => GetCollection<CuentaContable>();
